...
var regex ={
	/*
		...
		@param	{string}msg			값이 없을경우 표시될 메시지
	...
	*/
	...
	/*
		...
		@param	{int} len			최대 글자수
		@param	{string}msg			값이 없을 경우 표시될 메시지
		@return 지정된 글자수 이상인 경우	true / 초과한 경우 false
	*/
	...
	/*
		...
		@param	{string}selector	입력요소의 css셀렉터
		@param	{string}msg			값이 없을 경우 표시될 메시지
		@return 체크된경우	true / 초과한 경우 false
	*/
	check: function(selector,msg){
		var checked = $(selector + ':checked');		//대상요소중에서 선택된 항목들만 가져온다.
		if (checked.length <1 ) {					//선택된 항목의 길이가 1미만이라면?
			alert(msg);								//메시지를 표시한다
			$(selector).eq(0).focus();				//첫 번쨰 요소에게 포커스 강제 지정
			return false;				
		}
		return true;
	},
	/*
		체크박스나 라디오의 최소선택 갯수를 검사한다.
		@param	{string}selector	입력요소의 css셀렉터
		@param	{int} min			최소 선택 갯수
		@param	{string}msg			검사에 실패한 경우 표시할 메시지
		@return 최소 수량 이상인 경우	true / 초과한 경우 false
	*/
	check_min: function(selector, min, msg){
		//대상요소의 선택된 항목수가 주어진 min값보다 작다면?
	...
	/*
		...
		@param	{string}msg			검사에 실패한 경우 표시할 메시지
		@return 최소 최도 수량 이하인 경우	true / 그렇지 않은 경우 false
	*/
	...
	*/
	...
	/*
		입력값이 정규표현식을 충족하는지 검사한다.
 		@param	{string}selector	검사대상의의 selector
		@param	{object}regex_expr	검사 대상의 정규 표현식
		@param	{string}msg			검사에 실패한 경우 표시할 메시지
		@return 표현식을 충족할경우 	true / 그렇지 않은 경우 false
	*/
	field: function(selector, msg, regex_expr){
		var target = $(selector);	

		// 입력값이 없거나 입력값에 대한 정규 표현식 검사가 실패라면?
	...
	/*
		핸드폰번호 형식과 집전화 번호 형식 둘중 하나를 충족하는지 검사
 		@param	{string}selector	검사대상의의 selector
		@param	{string}msg			검사에 실패한 경우 표시할 메시지
		@return 조건에 충족할경우 	true / 그렇지 않은 경우 false
	*/
	phone: function(selector,msg){
		var check1 = /^01(?:0|1|[6-9])(?:\d{3}|\d{4})\d{4}$/; //핸드폰 형식
		var check2 = /^\d{2,3}\d{3,4}\d{4}$/;				  //집전 형식
		var target = $(selector);

		//입력값이 없거나, 핸드폰형식도 아니고 집전화 형식도 아니라면?
